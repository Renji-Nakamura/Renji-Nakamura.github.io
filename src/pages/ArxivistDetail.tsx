...
import ArxivistDeliverable from '@/components/ArxivistDeliverable';
import { playTerminalBeep } from '@/lib/mfp/useTerminalSound';

export default function ArxivistDetail() {
  // Ensure the page starts at the top when entering the detail view
  useLayoutEffect(() => {
    try {
      sessionStorage.setItem('portfolio_carousel_active_id', 'arxivist');
    } catch {
    ...
    >
      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12 mb-2">
        <Link 
          ...
          state={{ scrollTo: 'projects', projectId: 'arxivist' }}
          className="inline-flex items-center space-x-2 text-sm font-medium text-muted-foreground hover:text-red-500 transition-colors py-2 group outline-none"
        >
